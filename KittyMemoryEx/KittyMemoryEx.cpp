...
std::vector<ProcMap> getAllMaps(pid_t pid) {
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    sscanf(line, "%" SCNxPTR "-%" SCNxPTR " %4s %" SCNxPTR " %s %lu %s", &map.startAddress,
...
ProcMap getAddressMap(pid_t pid, uintptr_t address, const std::vector<ProcMap>& maps) {
  ...
  if (!maps.empty()) {
    auto it = std::lower_bound(maps.begin(), maps.end(), address,
                               [](const ProcMap& m, uintptr_t val) { return m.endAddress <= val; });
    ...
    auto it = std::lower_bound(pmaps.begin(), pmaps.end(), address,
                               [](const ProcMap& m, uintptr_t val) { return m.endAddress <= val; });
