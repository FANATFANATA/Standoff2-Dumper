...
std::string Dumper::getTypeIndexStr(Il2Cpp::Type* type) {
  ...
  switch (type_index) {
    ...
    case 0x15:
      return result + g().il2cpp->getTypeName(type);
    default:
      break;
  }
  return result + "object";
...
std::string Dumper::dumpField(Il2Cpp::Field* field, bool is_enum, bool is_struct) {
  ...
  if (g().memory->isPtrValid(reinterpret_cast<uintptr_t>(type))) {
    ...
    if (g().il2cpp->getAttrs(type) & 0x40) {
      ...
      if (g().memory->isPtrValid(reinterpret_cast<uintptr_t>(default_value)) &&
          g().memory->isPtrValid(reinterpret_cast<uintptr_t>(default_type))) {
        std::stringstream ss;
        ...
        switch (g().il2cpp->getTypeIndex(default_type)) {
          ...
          case 0xE: {
            ...
            size_t start = 0;
            while (s_ptr[start] != '\0' && (static_cast<unsigned char>(s_ptr[start]) <= 31 ||
                                            static_cast<unsigned char>(s_ptr[start]) >= 127)) {
              start++;
            }

            size_t end = start;
            while (s_ptr[end] != '\0' && static_cast<unsigned char>(s_ptr[end]) > 31 &&
                   static_cast<unsigned char>(s_ptr[end]) < 127) {
              end++;
            }
    ...
    if (is_enum) {
      if (result.find("const") == std::string::npos) {
        ...
        result +=
            " // " + (std::stringstream() << "0x" << std::hex << std::uppercase << offset).str();
      }
    } else if (is_struct) {
...
std::string Dumper::dumpProperty(Il2Cpp::Property* property) {
  ...
  if (g().memory->isPtrValid(reinterpret_cast<uintptr_t>(getter))) {
    ...
    if (g().memory->isPtrValid(reinterpret_cast<uintptr_t>(return_type))) {
      ...
      result += " { get; ";
      if (g().memory->isPtrValid(
              reinterpret_cast<uintptr_t>(g().il2cpp->getPropertySetter(property)))) {
        result += "set; }";
      } else {
        result += "}";
      }
...
std::string Dumper::dumpMethod(Il2Cpp::Method* method) {
  ...
  result += "// RVA: " + (std::stringstream() << "0x" << std::hex << std::uppercase << rva).str() +
            " VA: " + (std::stringstream() << "0x" << std::hex << std::uppercase << va).str() +
            (g().il2cpp->getMethodSlot(method) != 65535
                 ? " Slot: " + std::to_string(g().il2cpp->getMethodSlot(method))
                 : "") +
            "\n\t";
  ...
  for (int parameter_index = 0; parameter_index < g().il2cpp->getParameterCount(method);
       parameter_index++) {
...
std::string Dumper::dumpClass(Il2Cpp::Class* klass) {
  ...
  if (g().memory->isPtrValid(reinterpret_cast<uintptr_t>(this_type))) {
    ...
    result +=
        g().il2cpp->getIsGeneric(klass) &&
                g().memory->isPtrValid(reinterpret_cast<uintptr_t>(g().il2cpp->getThisType(klass)))
            ? g().il2cpp->getTypeName(g().il2cpp->getThisType(klass))
            : g().il2cpp->getClassName(klass);
    ...
    if (!g().il2cpp->isClassEnum(klass) && g().il2cpp->getTypeIndex(this_type) != 0x11) {
      ...
      if (g().memory->isPtrValid(reinterpret_cast<uintptr_t>(parent))) {
        if (!strcmp(g().il2cpp->getClassName(parent), "Object") &&
            !strcmp(g().il2cpp->getClassNamespace(parent), "System")) {
        } else {
          Il2Cpp::Type* parent_type = g().il2cpp->getThisType(parent);
          if (g().memory->isPtrValid(reinterpret_cast<uintptr_t>(parent_type))) {
            result += " : " + g().il2cpp->getTypeName(parent_type) +
                      (g().il2cpp->getInterfaces(klass).size() > 0 ? ", " : "");
          }
        }
      }
      ...
      for (int interface_index = 0; interface_index < g().il2cpp->getInterfaces(klass).size();
           interface_index++) {
    ...
    for (Il2Cpp::Field* field : g().il2cpp->getFields(klass)) {
      result += "\t" +
                dumpField(field, g().il2cpp->isClassEnum(klass),
                          g().il2cpp->getTypeIndex(this_type) == 0x11) +
                "\n";
    }
    ...
    if (!g().il2cpp->isClassEnum(klass)) {
      ...
      for (int method_index = 0; method_index < g().il2cpp->getMethods(klass).size();
           method_index++) {
        const auto methods = g().il2cpp->getMethods(klass);
