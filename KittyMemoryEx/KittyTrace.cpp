...
kitty_rp_call_t KittyTraceMgr::_callFunctionFrom(uintptr_t callerAddress, uintptr_t functionAddress,
                                                 int nargs, ...) {
  ...
  if (nargs > KT_REG_ARGS_NUM) {
    ...
    if (!pokeMem(tmp_regs.KT_REG_SP, &vargs[KT_REG_ARGS_NUM],
                 sizeof(uintptr_t) * (nargs - KT_REG_ARGS_NUM)))
...
#error "Unsupported ABI"
  ...
  do {
    ...
    if (wp != _pid) {
      ...
      KITTY_LOGE("callFunction(%p): waitpid return %d. \"%s\".", (void*)functionAddress, wp,
                 strerror(errno));
    ...
    if (WIFEXITED(status)) {
      _attached = false;
      KITTY_LOGE("callFunction(%p): Target process exited (%d).", (void*)functionAddress,
                 WEXITSTATUS(status));
    ...
    if (WIFSIGNALED(status)) {
      _attached = false;
      KITTY_LOGE("callFunction(%p): Target process terminated (%d).", (void*)functionAddress,
                 WTERMSIG(status));
      return {KT_RP_CALL_EXITED, {0}};
    }
    ...
    KITTY_LOGE(
        "callFunction(%p): Process didn't jump to specified return "
        "address (%p)",
        (void*)functionAddress, (void*)callerAddress);

    KITTY_LOGE("callFunction(%p): PC(%p) | RET(%p).", (void*)functionAddress,
               (void*)(return_regs.KT_REG_PC), (void*)(return_regs.KT_REG_RET));
    ...
    if (map.isValid()) {
      KITTY_LOGE("callFunction(%p): Fault Map(<base>+%p) %s", (void*)functionAddress,
    ...
    if (map.isValid()) {
      KITTY_LOGE("callFunction(%p): PC Map(<base>+%p) %s", (void*)functionAddress,
  ...
  KITTY_LOGD("callFunction: Calling function %p returned %p.", (void*)functionAddress,
             (void*)result.result.ptr);
...
kitty_rp_call_t KittyTraceMgr::_callSyscall(long sysnr, int nargs, ...) {
  ...
  KITTY_LOGD("callSyscall(%d, 0x%zx, 0x%zx, 0x%zx, 0x%zx, 0x%zx, 0x%zx)", int(sysnr), vargs[0],
  ...
  target_pc_mem &= ~1;
  if (thumb)
    syscall_code.assign(std::begin(KittyTraceInsns::THUMB_SYSCALL),
                        std::end(KittyTraceInsns::THUMB_SYSCALL));
  else
  ...
  if (!peekMem(target_pc_mem, backup_code.data(), backup_code.size())) {
    KITTY_LOGE("callSyscall(%d): Failed to backup PC(%p) memory code.", int(sysnr),
               (void*)target_pc_mem);
  ...
  do {
    ...
    if (return_regs.KT_REG_PC > tmp_regs.KT_REG_PC &&
        return_regs.KT_REG_PC <= tmp_regs.KT_REG_PC + 16)
      break;
    ...
    KITTY_LOGE("callSyscall(%d): SIG(%s) | CODE(%d) | ADDR(%p).", int(sysnr),
               strsignal(si.si_signo), si.si_code, (void*)(si.si_addr));
  ...
  if (!pokeMem(target_pc_mem, backup_code.data(), backup_code.size())) {
    KITTY_LOGW("callSyscall(%d): Failed to restore PC(%p) memory code!", int(sysnr),
               (void*)target_pc_mem);
  }
...
KT_BP_RESULT KittyTraceMgr::setSoftBreakpointAndWait(
    uintptr_t address, const std::function<bool(user_regs_struct regs)>& cb, int timeout_ms) {
...
again:
  ...
  if (!pokeMem(address, brk_code.data(), brk_code.size())) {
    KITTY_LOGE("setSoftBreakpointAndWait(%p): Failed to write brk code into memory.",
               (void*)address);
    return KT_BP_MEM_FAILED;
  }
  ...
  do {
    ...
    if (wp != tid) {
      ...
      KITTY_LOGE("setSoftBreakpointAndWait(%p): waitpid returned %d. \"%s\".", (void*)address, wp,
                 strerror(errno));
    ...
    if (WIFEXITED(status)) {
      _attached = false;
      KITTY_LOGE("setSoftBreakpointAndWait(%p): Target process exited (%d).", (void*)address,
                 WEXITSTATUS(status));
      return KT_BP_EXITED;
    }
    ...
    if (WIFSIGNALED(status)) {
      _attached = false;
      KITTY_LOGE("setSoftBreakpointAndWait(%p): Target process terminated (%d).", (void*)address,
                 WTERMSIG(status));
      return KT_BP_EXITED;
    }
    ...
    if (WSTOPSIG(status) == SIGTRAP) {
      ...
      KITTY_LOGE(
          "setSoftBreakpointAndWait(%p): Process didn't stop at "
          "specified Hardware Breakpoint",
          (void*)address);
    } else {
      KITTY_LOGE(
          "setSoftBreakpointAndWait(%p): Target process didn't stop "
          "with SIGTRAP",
          (void*)address);
    }

    KITTY_LOGE("setSoftBreakpointAndWait(%p): PC(%p) | RET(%p).", (void*)address,
               (void*)(regs.KT_REG_PC), (void*)(regs.KT_REG_RET));

    siginfo_t si = {};
    getSignalInfo(&si);

    KITTY_LOGE("setSoftBreakpointAndWait(%p): SIG(%s) | CODE(%d) | ADDR(%p).", (void*)address,
               strsignal(si.si_signo), si.si_code, (void*)(si.si_addr));

    auto map = KittyMemoryEx::getAddressMap(_pid, uintptr_t(si.si_addr));
    if (map.isValid()) {
      KITTY_LOGE("setSoftBreakpointAndWait(%p): MAP(<base>+%p) %s", (void*)address,
                 (void*)((map.offset + uintptr_t(si.si_addr)) - map.startAddress),
                 map.toString().c_str());
    }
  ...
  KITTY_LOGD("setSoftBreakpointAndWait(%p): Success PC(%p).", (void*)address,
             (void*)regs.KT_REG_PC);
...
KT_BP_RESULT KittyTraceMgr::setHardBreakpointAndWait(
    uintptr_t address, KT_HW_BP_TYPE type, KT_HW_BP_SIZE size, int slot,
    const std::function<bool(user_regs_struct regs)>& cb, int timeout_ms) {
  if (!_attached || _pid <= 0 || address == 0)
    return KT_BP_FAILED;
  ...
  auto validate_trap = [this, size](const user_regs_struct& regs, uintptr_t trap_addr) -> bool {
    ...
    uintptr_t pc = regs.KT_REG_PC;
    uintptr_t max_range =
        KT_ALIGN_UP(trap_addr + std::max(int(size), int(sizeof(KittyTraceInsns::BRKP))),
...
again:
...
#if defined(__arm__)
    if (!setHwBreakpoint(address, type, (address & 1) != 0 ? KT_HW_BP_SIZE_2 : KT_HW_BP_SIZE_4,
                         slot))
#elif defined(__aarch64__)
...
#endif
    {
      KITTY_LOGE("setHardBreakpointAndWait(%p): Failed to set breakpoint. \"%s\"", (void*)address,
                 strerror(errno));
      return KT_BP_FAILED;
    }
  } else {
    if (!setHwBreakpoint(address, type, size, slot)) {
      KITTY_LOGE("setHardBreakpointAndWait(%p): Failed to set watchpoint. \"%s\"", (void*)address,
                 strerror(errno));
      return KT_BP_FAILED;
    }
  }
  ...
  do {
    ...
    if (wp != tid) {
      ...
      KITTY_LOGE("setHardBreakpointAndWait(%p): waitpid returned %d. \"%s\".", (void*)address, wp,
                 strerror(errno));
    ...
    if (WIFEXITED(status)) {
      _attached = false;
      KITTY_LOGE("setHardBreakpointAndWait(%p): Target process exited (%d).", (void*)address,
                 WEXITSTATUS(status));
      return KT_BP_EXITED;
    }
    ...
    if (WIFSIGNALED(status)) {
      _attached = false;
      KITTY_LOGE("setHardBreakpointAndWait(%p): Target process terminated (%d).", (void*)address,
                 WTERMSIG(status));
      return KT_BP_EXITED;
    }
    ...
    if (WSTOPSIG(status) == SIGTRAP) {
      ...
      KITTY_LOGE(
          "setHardBreakpointAndWait(%p): Process didn't stop at "
          "specified Hardware Breakpoint",
          (void*)address);
    } else {
      KITTY_LOGE(
          "setHardBreakpointAndWait(%p): Target process didn't stop "
          "with SIGTRAP",
          (void*)address);
    }

    KITTY_LOGE("setHardBreakpointAndWait(%p): PC(%p) | RET(%p).", (void*)address,
               (void*)(regs.KT_REG_PC), (void*)(regs.KT_REG_RET));

    siginfo_t si = {};
    getSignalInfo(&si);

    KITTY_LOGE("setHardBreakpointAndWait(%p): SIG(%s) | CODE(%d) | ADDR(%p).", (void*)address,
               strsignal(si.si_signo), si.si_code, (void*)(si.si_addr));

    auto map = KittyMemoryEx::getAddressMap(_pid, uintptr_t(si.si_addr));
    if (map.isValid()) {
      KITTY_LOGE("setHardBreakpointAndWait(%p): MAP(<base>+%p) %s", (void*)address,
                 (void*)((map.offset + uintptr_t(si.si_addr)) - map.startAddress),
                 map.toString().c_str());
    }
  ...
  KITTY_LOGD("setHardBreakpointAndWait(%p): Success PC(%p).", (void*)address,
             (void*)regs.KT_REG_PC);
...
bool KittyTraceMgr::setHwBreakpoint(uintptr_t address, KT_HW_BP_TYPE type, KT_HW_BP_SIZE size,
                                    int slot) {
  errno = 0;
  pid_t tid = _pid;
  ...
  size_t alignment_mask =
      type == KT_HW_BP_EXECUTE ? (sizeof(uint32_t) - 1) : (sizeof(uintptr_t) - 1);
  ...
  iov.iov_base = &state;
  iov.iov_len =
      offsetof(struct user_hwdebug_state, dbg_regs) + (sizeof(state.dbg_regs[0]) * (slot + 1));
