...
#define KITTY_LOGD(fmt, ...) \
  ((void)__android_log_print(ANDROID_LOG_DEBUG, KITTY_LOG_TAG, fmt, ##__VA_ARGS__))
#else
#define KITTY_LOGD(fmt, ...) \
  do {                       \
  } while (0)
...
#define KITTY_LOGI(fmt, ...) \
  ((void)__android_log_print(ANDROID_LOG_INFO, KITTY_LOG_TAG, fmt, ##__VA_ARGS__))
#define KITTY_LOGE(fmt, ...) \
  ((void)__android_log_print(ANDROID_LOG_ERROR, KITTY_LOG_TAG, fmt, ##__VA_ARGS__))
#define KITTY_LOGW(fmt, ...) \
  ((void)__android_log_print(ANDROID_LOG_WARN, KITTY_LOG_TAG, fmt, ##__VA_ARGS__))
...
inline T getSystemProperty(const std::string& key, T defaultValue) {
  static_assert(std::is_same_v<T, std::string> || std::is_same_v<T, bool> ||
                ...
                "getSystemProperty: unsupported type. Supported types: string, "
                "bool, integral, floating-point.");
  ...
  if constexpr (std::is_same_v<T, std::string>) {
    ...
    for (int i = 0; i < len; ++i) value[i] = std::tolower(value[i]);

    if (std::strcmp(value, "1") == 0 || std::strcmp(value, "true") == 0 ||
        std::strcmp(value, "y") == 0 || std::strcmp(value, "yes") == 0)
      return true;

    if (std::strcmp(value, "0") == 0 || std::strcmp(value, "false") == 0 ||
        std::strcmp(value, "n") == 0 || std::strcmp(value, "no") == 0)
      return false;
 ...
 *
 * @return Absolute path to external storage root, falls back to "/sdcard" if
 * not defined.
 */
 ...
 * If TMPDIR is not set, it falls back to
 ...
 * @param sensitive Whether the comparison should be case-sensitive (default is
 * true).
 *
 ...
 * true).
 *
 * @return true if str starts with at least one prefix in prefixes, false
 * otherwise.
 */
 ...
 * true).
 *
 * @return true if str contains at least one substring in substrings, false
 * otherwise.
 */
bool contains(const std::string& str, const std::vector<std::string>& substrings,
              bool sensitive = true);
 ...
 * true).
 *
 * @return true if str ends with at least one suffix in suffixes, false
 * otherwise.
 */
 ...
 * @param data Pointer to the destination buffer where the binary data will be
 * stored.
 *
 ...
 * @tparam showASCII Whether to include ASCII representation of the memory
 * block. Defult is true.
 *
 ...
 * It prints the address, hexadecimal values, and ASCII representation of the
 * block. The dump is formatted into rows of specified size, and each row
 * includes the offset, byte values, and ASCII characters. The ASCII
 * representation only includes printable characters, and non-printable
 ...
 * @param totalEntries Pointer to store the total number of entries in the ZIP
 * file.
 *
 * @return True if the central directory is found, false otherwise.
 */
bool findCentralDirectory(const uint8_t* data, uint64_t fileSize, uint64_t* cdOffset,
 ...
 *
 * @return A vector of ZipEntryInfo objects containing information about each
 * entry.
 */
