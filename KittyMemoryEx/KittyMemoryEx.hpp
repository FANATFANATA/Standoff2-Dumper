...
public:
  ...
  inline bool operator==(const ProcMap& other) const {
    return (pid == other.pid && startAddress == other.startAddress &&
            endAddress == other.endAddress && protection == other.protection &&
            is_private == other.is_private && is_shared == other.is_shared &&
            offset == other.offset && dev == other.dev && inode == other.inode &&
  ...
  inline std::string toString() const {
    return KittyUtils::String::fmt(
        "%" PRIxPTR "-%" PRIxPTR " %c%c%c%c %" PRIxPTR " %s %lu %s", startAddress, endAddress,
 ...
 *
 * This function reads the command line of the process identified by the given
 * process ID and returns the process name.
 *
 ...
 *
 * This function searches for all processes whose command line matches the
 * specified process name and returns their process IDs in a vector.
 *
 ...
 *
 * This function searches for the first process whose command line matches the
 * specified process name and returns its process ID.
 *
 * @param processName The name of the process to match.
 ...
 *
 * This function retrieves the thread IDs of all threads associated with the
 * specified process ID by examining the `/proc/[pid]/task` directory.
 *
 * @param pid The process ID of the process.
...
public:
   ...
   * @param out A pointer to the ProcStatus object where the parsed data will be
   * stored.
   ...
   *
   * If the tid is less than or equal to 0, it calls the parse function with the
   * pid. If the tid is greater than 0, it calls the parse function with the pid
   * and tid.
   *
   ...
   *
   * @param key The key to retrieve.
   * @return The integer value associated with the key, or 0 if the key is not
   * found.
   */
 ...
 */
enum class EProcMapFilter { Equal, Contains, StartWith, EndWith, Regex };
...
/**
 * @brief Retrieves the map information for a specific address in the current
 * process.
 *
 * @param pid The remote process ID
 * @param address Address to search for.
 * @param maps The vector of cached process maps (optional).
 * @return ProcMap object representing the map for the address, or an invalid
 * map if not found.
 */
