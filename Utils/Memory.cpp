...
std::vector<uintptr_t> Memory::ModuleInfo::findPattern(const std::string& pattern,
                                                       bool need_first_only) const {
...
bool Memory::initialize() {
  ...
  getModuleInfo("libunity.so", [&](const ModuleInfo& module) {
    ...
    auto log_progress = [&](const char* name, uintptr_t address) {
      ...
      g().log->info("[%s] %3d%% | Finished: %s (0x%lX)", bar.c_str(), percent, name,
...
void Memory::findPatternBasedOffsets(
    const ModuleInfo& module, const std::function<void(const char*, uintptr_t)>& log_progress) {
  if (offsets.assemblies_start == UINTPTR_MAX)
    offsets.assemblies_start = findAssembliesStart(module);
  log_progress("assemblies_start", offsets.assemblies_start);
  if (offsets.assemblies_end == UINTPTR_MAX)
    offsets.assemblies_end = findAssembliesEnd(module);
  log_progress("assemblies_end", offsets.assemblies_end);

  if (offsets.assembly_open == UINTPTR_MAX)
    offsets.assembly_open = findAssemblyOpen(module);
  log_progress("assembly_open", offsets.assembly_open);
  if (offsets.assembly_name == UINTPTR_MAX)
    offsets.assembly_name = findAssemblyName(module);
  log_progress("assembly_name", offsets.assembly_name);
  if (offsets.assembly_get_image == UINTPTR_MAX)
    offsets.assembly_get_image = findAssemblyGetImage(module);
  log_progress("assembly_get_image", offsets.assembly_get_image);

  if (offsets.image_type_count == UINTPTR_MAX)
    offsets.image_type_count = findImageTypeCount(module);
  log_progress("image_type_count", offsets.image_type_count);
  if (offsets.image_get_assembly_type_handle == UINTPTR_MAX)
    offsets.image_get_assembly_type_handle = findGetAssemblyTypeHandle(module);
  log_progress("image_get_assembly_type_handle", offsets.image_get_assembly_type_handle);
  if (offsets.handle_get_type_from_handle == UINTPTR_MAX)
    offsets.handle_get_type_from_handle = findGetTypeFromHandle(module);
  log_progress("handle_get_type_from_handle", offsets.handle_get_type_from_handle);
  if (offsets.image_name == UINTPTR_MAX)
    offsets.image_name = findImageName(module);
  log_progress("image_name", offsets.image_name);

  if (offsets.class_get_name == UINTPTR_MAX)
    offsets.class_get_name = findClassGetName(module);
  log_progress("class_get_name", offsets.class_get_name);
  if (offsets.class_get_parent == UINTPTR_MAX)
    offsets.class_get_parent = findClassGetParent(module);
  log_progress("class_get_parent", offsets.class_get_parent);
  if (offsets.class_get_fields == UINTPTR_MAX)
    offsets.class_get_fields = findClassGetFields(module);
  log_progress("class_get_fields", offsets.class_get_fields);
  if (offsets.class_get_properties == UINTPTR_MAX)
    offsets.class_get_properties = findClassGetProperties(module);
  log_progress("class_get_properties", offsets.class_get_properties);
  if (offsets.class_get_methods == UINTPTR_MAX)
    offsets.class_get_methods = findClassGetMethods(module);
  log_progress("class_get_methods", offsets.class_get_methods);
  if (offsets.class_type == UINTPTR_MAX)
    offsets.class_type = findClassType(module);
  log_progress("class_type", offsets.class_type);
  if (offsets.class_get_enum_basetype == UINTPTR_MAX)
    offsets.class_get_enum_basetype = findClassGetEnumBasetype(module);
  log_progress("class_get_enum_basetype", offsets.class_get_enum_basetype);
  if (offsets.class_get_flags == UINTPTR_MAX)
    offsets.class_get_flags = findClassGetFlags(module);
  log_progress("class_get_flags", offsets.class_get_flags);
  if (offsets.class_get_interfaces == UINTPTR_MAX)
    offsets.class_get_interfaces = findClassGetInterfaces(module);
  log_progress("class_get_interfaces", offsets.class_get_interfaces);
  if (offsets.class_get_declaring_type == UINTPTR_MAX)
    offsets.class_get_declaring_type = findClassGetDeclaringType(module);
  log_progress("class_get_declaring_type", offsets.class_get_declaring_type);
  if (offsets.class_get_is_generic == UINTPTR_MAX)
    offsets.class_get_is_generic = findClassGetIsGeneric(module);
  log_progress("class_get_is_generic", offsets.class_get_is_generic);

  if (offsets.field_get_name == UINTPTR_MAX)
    offsets.field_get_name = findFieldGetName(module);
  log_progress("field_get_name", offsets.field_get_name);
  if (offsets.field_get_offset == UINTPTR_MAX)
    offsets.field_get_offset = findFieldGetOffset(module);
  log_progress("field_get_offset", offsets.field_get_offset);
  if (offsets.field_get_type == UINTPTR_MAX)
    offsets.field_get_type = findFieldGetType(module);
  log_progress("field_get_type", offsets.field_get_type);
  if (offsets.field_get_default_value == UINTPTR_MAX)
    offsets.field_get_default_value = findFieldGetDefaultValue(module);
  log_progress("field_get_default_value", offsets.field_get_default_value);

  if (offsets.type_get_type == UINTPTR_MAX)
    offsets.type_get_type = findTypeGetType(module);
  log_progress("type_get_type", offsets.type_get_type);
  if (offsets.type_attrs == UINTPTR_MAX)
    offsets.type_attrs = findTypeAttrs(module);
  log_progress("type_attrs", offsets.type_attrs);
  if (offsets.type_attrs_shift == UINTPTR_MAX)
    offsets.type_attrs_shift = findTypeAttrsShift(module);
  log_progress("type_attrs_shift", offsets.type_attrs_shift);
  if (offsets.type_get_class_from_type == UINTPTR_MAX)
    offsets.type_get_class_from_type = findTypeGetClassFromType(module);
  log_progress("type_get_class_from_type", offsets.type_get_class_from_type);
  if (offsets.type_get_data == UINTPTR_MAX)
    offsets.type_get_data = findTypeGetData(module);
  log_progress("type_get_data", offsets.type_get_data);
  if (offsets.type_get_name == UINTPTR_MAX)
    offsets.type_get_name = findTypeGetName(module);
  log_progress("type_get_name", offsets.type_get_name);

  if (offsets.method_get_flags == UINTPTR_MAX)
    offsets.method_get_flags = findMethodGetFlags(module);
  log_progress("method_get_flags", offsets.method_get_flags);
  if (offsets.method_parameters_count == UINTPTR_MAX)
    offsets.method_parameters_count = findMethodParametersCount(module);
  log_progress("method_parameters_count", offsets.method_parameters_count);
  if (offsets.method_get_parameter == UINTPTR_MAX)
    offsets.method_get_parameter = findMethodGetParameter(module);
  log_progress("method_get_parameter", offsets.method_get_parameter);
  if (offsets.method_pointer == UINTPTR_MAX)
    offsets.method_pointer = findMethodPointer(module);
  log_progress("method_pointer", offsets.method_pointer);
  if (offsets.method_virtual_pointer == UINTPTR_MAX)
    offsets.method_virtual_pointer = findMethodVirtualPointer(module);
  log_progress("method_virtual_pointer", offsets.method_virtual_pointer);
  if (offsets.method_slot == UINTPTR_MAX)
    offsets.method_slot = findMethodSlot(module);
  log_progress("method_slot", offsets.method_slot);
  if (offsets.method_get_is_generic == UINTPTR_MAX)
    offsets.method_get_is_generic = findMethodGetIsGeneric(module);
  log_progress("method_get_is_generic", offsets.method_get_is_generic);
  if (offsets.method_get_parameter_name == UINTPTR_MAX)
    offsets.method_get_parameter_name = findMethodGetParameterName(module);
  log_progress("method_get_parameter_name", offsets.method_get_parameter_name);
}

void Memory::findIl2CppBasedOffsets(
    const ModuleInfo& module, const std::function<void(const char*, uintptr_t)>& log_progress) {
  if (offsets.class_namespace == UINTPTR_MAX)
    offsets.class_namespace = findClassNamespace(module);
  log_progress("class_namespace", offsets.class_namespace);
  if (offsets.property_name == UINTPTR_MAX)
    offsets.property_name = findPropertyName(module);
  log_progress("property_name", offsets.property_name);
  if (offsets.property_get_method == UINTPTR_MAX)
    offsets.property_get_method = findPropertyGetMethod(module);
  log_progress("property_get_method", offsets.property_get_method);
  if (offsets.property_set_method == UINTPTR_MAX)
    offsets.property_set_method = findPropertySetMethod(module);
  log_progress("property_set_method", offsets.property_set_method);
  if (offsets.method_name == UINTPTR_MAX)
    offsets.method_name = findMethodName(module);
  log_progress("method_name", offsets.method_name);
  if (offsets.method_return_type == UINTPTR_MAX)
    offsets.method_return_type = findMethodReturnType(module);
  log_progress("method_return_type", offsets.method_return_type);
}

Memory::ModuleInfo Memory::getModuleInfo(const char* name,
                                         const std::function<void(ModuleInfo)>& callback) {
  ModuleInfo info = {};
  ...
  while (fgets(line, sizeof line, f)) {
    ...
    if (sscanf(line, "%" PRIXPTR "-%" PRIXPTR " %4s %*s %*s %*s %255s", &tmp_base, &tmp_end, perms,
               path) > 0) {
      if (!strcmp(basename(path), name)) {
        int permissions = (perms[0] == 'r' ? (1 << 1) : 0) | (perms[1] == 'w' ? (1 << 2) : 0) |
                          (perms[2] == 'x' ? (1 << 3) : 0) | (perms[3] == 'p' ? (1 << 4) : 0);
...
std::string Memory::dumpHex(uintptr_t address, size_t size) {
  ...
  for (size_t i = 0; i < size; i += 16) {
    ...
    for (size_t j = 0; j < 16; ++j) {
      if (i + j < size) {
        ss << std::setw(2) << std::setfill('0') << std::hex << static_cast<int>(p[i + j]) << " ";
      } else {
        ss << "   ";
      }
    }
...
void Memory::dumpHexToFile(uintptr_t address, size_t size, const std::string& filePath) {
  ...
  for (size_t i = 0; i < size; i += 8) {
    ...
    for (size_t j = 0; j < 8; ++j) {
      if (i + j < size) {
        outFile << std::setw(2) << std::setfill('0') << std::hex << static_cast<int>(p[i + j])
                << " ";
      } else {
        outFile << "   ";
      }
...
uintptr_t Memory::findAssemblyOpen(ModuleInfo module) {
  auto pattern_result =
      module.findPattern("? ? ? F8 ? ? ? A9 ? ? ? A9 F3 03 00 AA ? ? ? 95 ? ? ? ? ? ? ? 91", false);
  if (pattern_result.empty())
    return UINTPTR_MAX;
...
uintptr_t Memory::findAssemblyName(ModuleInfo module) {
  auto pattern_result = module.findPattern(
      "? ? ? F8 ? ? ? A9 ? ? ? A9 F3 03 00 "
      "AA ? ? ? 95 ? ? ? ? ? ? ? 91");
  if (pattern_result.empty())
    return UINTPTR_MAX;
  ...
  if (g().memory->isPtrValid(reinterpret_cast<uintptr_t>(object))) {
    for (uintptr_t offset = 0; offset < 512; offset += 8) {
      const char* namespaze =
          *reinterpret_cast<const char**>(reinterpret_cast<uintptr_t>(object) + offset);
...
uintptr_t Memory::findClassGetFlags(ModuleInfo module) {
  auto pattern_result = module.findPattern(
      "? ? ? B9 C0 03 5F D6 ? ? ? 39 ? ? ? 53 C0 03 5F D6 ? "
      "? ? 39 ? ? ? 37 ? ? ? 39");
  if (pattern_result.empty())
    return UINTPTR_MAX;
  return pattern_result.front();
...
uintptr_t Memory::findClassGetDeclaringType(ModuleInfo module) {
  auto pattern_result =
      module.findPattern("? ? ? F9 C0 03 5F D6 ? ? ? F8 ? ? ? A9 ? ? ? 79", false);
  if (pattern_result.empty())
    return UINTPTR_MAX;
  return pattern_result.back();
}
...
uintptr_t Memory::findClassGetIsGeneric(ModuleInfo module) {
  auto pattern_result =
      module.findPattern("? ? ? F8 ? ? ? A9 ? ? ? A9 ? ? ? A9 ? ? ? F9 F3 03 01 2A ? ? ? 52");
  if (pattern_result.empty())
    return UINTPTR_MAX;
  uintptr_t function_address = g().memory->getUnityBase() + pattern_result.front();
...
uintptr_t Memory::findFieldGetDefaultValue(ModuleInfo module) {
  auto pattern_result = module.findPattern(
      "? ? ? D1 ? ? ? F9 ? ? ? A9 ? ? ? A9 ? ? ? F9 F4 03 "
      "00 AA F3 03 01 AA ? ? ? F9");
  if (pattern_result.empty())
    return UINTPTR_MAX;
  return pattern_result.front();
}
...
uintptr_t Memory::findTypeAttrs(ModuleInfo module) {
  ...
  Assembler::bl first_bl =
      Assembler::bl(first_bl_address, *reinterpret_cast<uint32_t*>(first_bl_address));

  uintptr_t second_function_address = first_bl.target;
  uintptr_t second_bl_address = second_function_address + 0x60;
  Assembler::bl second_bl =
      Assembler::bl(second_bl_address, *reinterpret_cast<uint32_t*>(second_bl_address));

  uintptr_t third_function_address = second_bl.target;
  uintptr_t third_bl_address = third_function_address + 0xC4;
  Assembler::bl third_bl =
      Assembler::bl(third_bl_address, *reinterpret_cast<uint32_t*>(third_bl_address));
  ...
  Assembler::ldr ldr =
      Assembler::ldr(root_ldr_address, *reinterpret_cast<uint32_t*>(root_ldr_address));
  return ldr.offset;
}
...
uintptr_t Memory::findTypeAttrsShift(ModuleInfo module) {
  ...
  Assembler::ubfx ubfx =
      Assembler::ubfx(root_ubfx_address, *reinterpret_cast<uint32_t*>(root_ubfx_address));
  return ubfx.isValid() ? ubfx.lsb : 0;
}

uintptr_t Memory::findTypeGetClassFromType(ModuleInfo module) {
  auto pattern_result =
      module.findPattern("? ? ? A9 ? ? ? A9 ? ? ? 39 F3 03 00 AA F4 03 01 2A E0 03 15 2A");
  if (pattern_result.empty())
    return UINTPTR_MAX;
  return pattern_result.front();
}
...
uintptr_t Memory::findTypeGetData(ModuleInfo module) {
  ...
  if (class_from_type == UINTPTR_MAX)
    return UINTPTR_MAX;
  ...
  if (g().memory->isPtrValid(reinterpret_cast<uintptr_t>(object))) {
    ...
    if (g().memory->isPtrValid(reinterpret_cast<uintptr_t>(property))) {
      for (uintptr_t offset = 0; offset < 64; offset += 8) {
        const char* name =
            *reinterpret_cast<const char**>(reinterpret_cast<uintptr_t>(property) + offset);
    ...
    if (g().memory->isPtrValid(reinterpret_cast<uintptr_t>(property))) {
      for (uintptr_t offset = 0; offset < 128; offset += 8) {
        Il2Cpp::Method* method =
            *reinterpret_cast<Il2Cpp::Method**>(reinterpret_cast<uintptr_t>(property) + offset);
        if (g().memory->isPtrValid(reinterpret_cast<uintptr_t>(method))) {
          for (uintptr_t method_offset = 0; method_offset < 128; method_offset += 8) {
            const char* name = *reinterpret_cast<const char**>(reinterpret_cast<uintptr_t>(method) +
                                                               method_offset);
            if (g().memory->isPtrValid(reinterpret_cast<uintptr_t>(name))) {
  ...
  if (g().memory->isPtrValid(reinterpret_cast<uintptr_t>(klass))) {
    for (Il2Cpp::Method* method : g().il2cpp->getMethods(klass)) {
      if (!strcmp(g().il2cpp->getMethodName(method), "GetEvents_native")) {
        for (uintptr_t offset = 0; offset < 128; offset += 8) {
          Il2Cpp::Type* t =
              *reinterpret_cast<Il2Cpp::Type**>(reinterpret_cast<uintptr_t>(method) + offset);
    ...
    for (Il2Cpp::Method* method : g().il2cpp->getMethods(klass)) {
      if (!strcmp(g().il2cpp->getMethodName(method), "get_method_attributes")) {
        for (uintptr_t offset1 = 0; offset1 < 128; offset1 += 8) {
          uintptr_t va =
              *reinterpret_cast<uintptr_t*>(reinterpret_cast<uintptr_t>(method) + offset1);
          if (g().memory->isPtrValid(va)) {
            for (uintptr_t offset2 = 0; offset2 < 128; offset2 += 8) {
              ...
              uintptr_t rva =
                  *reinterpret_cast<uintptr_t*>(reinterpret_cast<uintptr_t>(method) + offset2);
              if (va == rva) {
