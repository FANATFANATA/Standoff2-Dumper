...
std::string randomString(size_t length) {
  ...
  for (size_t i = 0; i < length; ++i) str[i] = chars[dist(rnd)];
...
bool String::contains(const std::string& str, const std::string& substring, bool sensitive) {
  ...
  auto it =
      std::search(str.begin(), str.end(), substring.begin(), substring.end(), charEqualsIgnoreCase);
  return it != str.end();
}
...
bool String::startsWith(const std::string& str, const std::vector<std::string>& prefixes,
                        bool sensitive) {
...
bool String::contains(const std::string& str, const std::vector<std::string>& substrings,
                      bool sensitive) {
...
bool String::endsWith(const std::string& str, const std::vector<std::string>& suffixes,
                      bool sensitive) {
...
std::vector<ZipEntryInfo> listEntriesInZip(const std::string& zipPath) {
  ...
  while (offset + KT_CENTRAL_DIR_SIZE <= fileSize) {
    ...
    info.fileName.assign(reinterpret_cast<const char*>(data + offset + KT_CENTRAL_DIR_SIZE),
                         nameLen);
