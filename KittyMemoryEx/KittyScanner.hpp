...
public:
  ...
  /**
   * @brief Searches for bytes within a remote memory range and returns all
   * results.
   *
  ...
  /**
   * @brief Searches for bytes within a remote memory range and returns the
   * first result.
   *
   * @param start: The starting address to search.
   * @param end: The ending address to search.
  ...
  /**
   * @brief Searches for hex within a remote memory range and returns all
   * results.
   *
   * @param start: The starting address to search.
   * @param end: The ending address to search.
  ...
  /**
   * @brief Searches for hex within a remote memory range and returns the first
   * result.
   *
   * @param start: The starting address to search.
   * @param end: The ending address to search.
  ...
  /**
   * @brief Searches for a pattern within a remote memory range using the IDA
   * pattern syntax and returns all results.
   *
   * @param start: The starting address to search.
   * @param end: The ending address to search.
  ...
  /**
   * @brief Searches for a pattern within a remote memory range using the IDA
   * pattern syntax and returns the first result.
   *
   * @param start: The starting address to search.
   * @param end: The ending address to search.
   * @param pattern: The IDA pattern string (e.g., "FF DD ? 99 CC ? 00").
  ...
  /**
   * @brief Searches for data within a remote memory range and returns all
   * results.
   *
   * @param start: The starting address to search.
   * @param end: The ending address to search.
  ...
  /**
   * @brief Searches for data within a remote memory range and returns the first
   * result.
   *
   * @param start: The starting address to search.
   * @param end: The ending address to search.
   ...
   */
  uintptr_t findDataFirst(const uintptr_t start, const uintptr_t end, const void* data,
                          size_t size) const;
};
 ...
 * This class is used to scan ELF files from memory and extract information
...
public:
   ...
   * This constructor initializes the ElfScanner class with the given ELF base
   * address.
   *
   * @param pMem Pointer to the memory operation interface.
  ...
  ElfScanner(IKittyMemOp* pMem, uintptr_t elfBase)
      : ElfScanner(pMem, elfBase,
                   (pMem ? KittyMemoryEx::getAllMaps(pMem->processID())
                         : std::vector<KittyMemoryEx::ProcMap>())) {
  }
   ...
   * \ref createWithSoInfo(const kitty_soinfo_t&, const
   * std::vector<KittyMemory::ProcMap>&) may fix header.
   */
   ...
   * @return A map where keys are symbol names and values are their
...
public:
  ...
  /**
   * @brief Checks if the memory-mapped ELF file is native to the Android
   * device.
  ...
  /**
   * @brief Checks if the memory-mapped ELF file is emulated to the Android
   * device.
   * @param elf The ELF scanner object.
   ...
   * @param type (optional) The type of ELF file to find (e.g., Any, Native,
   * Emulated).
   * @param filter (optional) The filter to apply when searching for ELF files
   * (e.g., Any, System, App).
  ...
  /**
   * @brief Searches for a memory mapped ELF file based on the given path, type,
   * and filter.
   * @note If multiple ELFs found, it will prioritize ELFs with dynamic and
   * return the latest mapped one with the most segments. This function can
   * extract full path to loaded zipped Elf files.
   *
   ...
   * @param filter (optional) The filter to apply when searching for ELF files.
   * (e.g., Any, System, App).
   * @return An ElfScanner object representing the found ELF file, or an empty
   * ElfScanner object if not found.
   */
   ...
   * @param filter (optional) The filter to apply when searching for ELF files.
   * (e.g., Any, System, App).
   * @return A vector of pairs containing the symbol's absolute address and the
   * ELF where the symbol was found.
   */
  std::vector<std::pair<uintptr_t, ElfScanner>> findSymbolAll(
      const std::string& symbolName, EScanElfType type = EScanElfType::Any,
      EScanElfFilter filter = EScanElfFilter::Any);
#else
  /**
   * @brief Fetches all memory mapped ELFs.
   ...
   *
   * This function searches for the remote address of a local symbol based on
...
public:
  ...
  inline uintptr_t solist() const {
    ...
    uintptr_t value = 0;
    return _pMem->Read(_linker_syms.solist, &value, sizeof(uintptr_t)) == sizeof(uintptr_t) ? value
                                                                                            : 0;
  }
  ...
  inline uintptr_t somain() const {
    ...
    uintptr_t value = 0;
    return _pMem->Read(_linker_syms.somain, &value, sizeof(uintptr_t)) == sizeof(uintptr_t) ? value
                                                                                            : 0;
  }
  ...
  inline uintptr_t sonext() const {
    ...
    uintptr_t value = 0;
    return _pMem->Read(_linker_syms.sonext, &value, sizeof(uintptr_t)) == sizeof(uintptr_t) ? value
                                                                                            : 0;
  }
...
private:
  kitty_soinfo_t infoFromSoInfo_(uintptr_t si,
...
struct nbItf_data_t {
  ...
  void* (*createNamespace)(const char* name, const char* ld_library_path,
                           const char* default_library_path, uint64_t type,
                           const char* permitted_when_isolated_path, void* parent_ns);
  ...
  void (*preZygoteFork)();
  void* (*getTrampolineWithJNICallType)(void* handle, const char* name, const char* shorty,
                                        uint32_t len, enum KT_JNICallType jni_call_type);
...
public:
  ...
  /**
   * @brief Getter for the native bridge implementaion Elf (e.g, 'libhoudini.so'
   * or 'libndk_translation.so').
   */
