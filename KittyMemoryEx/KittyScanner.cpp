...
uintptr_t findInRange(const uintptr_t start, const uintptr_t end, const uint8_t* pattern,
                      const char* mask) {
  ...
  while (anchor_index < mask_len && mask[anchor_index] != 'x') ++anchor_index;
...
std::vector<uintptr_t> KittyScannerMgr::findBytesAll(const uintptr_t start, const uintptr_t end,
  ...
  do {
    uintptr_t found = findInRange(curr_search_address, search_end,
                                  reinterpret_cast<const uint8_t*>(bytes), mask.data());
    if (!found)
      break;
...
uintptr_t KittyScannerMgr::findBytesFirst(const uintptr_t start, const uintptr_t end,
  ...
  uintptr_t local = findInRange((uintptr_t)&buf[0], (uintptr_t(&buf[0]) + buf.size()),
                                reinterpret_cast<const uint8_t*>(bytes), mask.data());
  return local ? ((local - (uintptr_t(&buf[0]))) + start) : 0;
}

std::vector<uintptr_t> KittyScannerMgr::findHexAll(const uintptr_t start, const uintptr_t end,
                                                   std::string hex, const std::string& mask) const {
...
uintptr_t KittyScannerMgr::findIdaPatternFirst(const uintptr_t start, const uintptr_t end,
                                               const std::string& pattern) {
  if (!_pMem || start >= end)
    return 0;
...
std::vector<uintptr_t> KittyScannerMgr::findDataAll(const uintptr_t start, const uintptr_t end,
...
uintptr_t KittyScannerMgr::findDataFirst(const uintptr_t start, const uintptr_t end,
                                         const void* data, size_t size) const {
  if (!_pMem || start >= end || !data || size < 1)
    return 0;
...
ElfScanner::ElfScanner(IKittyMemOp* pMem, uintptr_t elfBase,
                       const std::vector<KittyMemoryEx::ProcMap>& maps) {
  _pMem = nullptr;
  _elfBase = 0;
  _ehdr = {};
  _phdr = 0;
  ...
  if (_ehdr.e_type != ET_EXEC && _ehdr.e_type != ET_DYN) {
    KITTY_LOGD(
        "ElfScanner: (%p) is not a executable or dynamic "
        "library.",
        (void*)elfBase);
    return;
  }
  ...
  for (auto& phdr : _phdrs) {
    if (phdr.p_type == PT_DYNAMIC) {
      if (phdr.p_vaddr == 0 || phdr.p_memsz == 0)
        break;
      if (!KittyMemoryEx::getAddressMap(_pMem->processID(), _loadBias + phdr.p_vaddr, maps)
               .readable)
        break;
      if (!KittyMemoryEx::getAddressMap(_pMem->processID(),
                                        _loadBias + phdr.p_vaddr + (phdr.p_memsz - 1), maps)
               .readable)
        break;
...
#ifdef __ANDROID__
ElfScanner::ElfScanner(IKittyMemOp* pMem, const kitty_soinfo_t& soinfo,
                       const std::vector<KittyMemoryEx::ProcMap>& maps) {
  _pMem = nullptr;
  _elfBase = 0;
  _ehdr = {};
  ...
  if (!isLinker && (_elfBase == 0 || _loadSize == 0 || _loadBias == 0 || _phdr == 0 ||
                    _dynamic == 0 || _stringTable == 0 || _symbolTable == 0)) {
    KITTY_LOGD("ElfScanner: Invalid soinfo!");
    KITTY_LOGD(
        "ElfScanner: elfBase: %p | bias: %p | phdr: %p | dyn: %p | "
        "strtab=%p | symtab=%p | strsz=%p | syment=%p",
  ...
  // some games like farlight have corrupted header and needs to be fixed by
  // soinfo
  if (!isLinker &&
      (memcmp(_ehdr.e_ident, "\177ELF", 4) != 0 || _ehdr.e_ident[EI_CLASS] != KT_ELF_EICLASS ||
    ...
    KITTY_LOGD("ElfScanner: soinfo(%p) has corrupted header, fixing by soinfo...",
               (void*)soinfo.ptr);
  ...
  if (!isLinker) {
    ...
    if (!(dynMap.readable && dynMap.startAddress >= _elfBase &&
          dynMap.endAddress <= (_elfBase + _loadSize))) {
  ...
  if (!(biasMap.readable && biasMap.startAddress >= _elfBase &&
        biasMap.endAddress <= (_elfBase + _loadSize))) {
  ...
  for (auto& phdr : _phdrs) {
    if (phdr.p_type == PT_DYNAMIC) {
      ...
      if (!KittyMemoryEx::getAddressMap(_pMem->processID(), _dynamic, maps).readable)
        break;
      if (!KittyMemoryEx::getAddressMap(_pMem->processID(), _dynamic + (phdr.p_memsz - 1), maps)
               .readable)
        break;
...
std::unordered_map<std::string, uintptr_t> ElfScanner::dsymbols() {
  if (!_dsymbols_init && _loadBias && !_filepath.empty()) {
    ...
    if (isZipped()) {
      ...
      if (!elfFile.open()) {
        KITTY_LOGD("Failed to open file <%s> err(%s)", _filepath.c_str(),
                   elfFile.lastStrError().c_str());
        return _dsymbolsMap;
      }
    ...
    const KT_ElfW(Shdr)* shdr =
        reinterpret_cast<KT_ElfW(Shdr)*>(reinterpret_cast<char*>(mmap_info.data) + ehdr->e_shoff);
    const KT_ElfW(Shdr)* shstrtab_shdr = shdr + ehdr->e_shstrndx;
    const char* sectionstr =
        reinterpret_cast<char*>(reinterpret_cast<char*>(mmap_info.data) + shstrtab_shdr->sh_offset);
    for (uint16_t i = 0; i < ehdr->e_shnum; ++i) {
      if (shdr[i].sh_type != SHT_SYMTAB)
        continue;

      std::string section_name =
          std::string(reinterpret_cast<const char*>(sectionstr + shdr[i].sh_name));
      if (section_name.compare(".symtab") != 0)
        continue;

      if ((shdr[i].sh_offset + shdr[i].sh_size) > mmap_info.size ||
          shdr[i].sh_link >= ehdr->e_shnum ||
          (shdr[shdr[i].sh_link].sh_offset + shdr[shdr[i].sh_link].sh_size) > mmap_info.size)
        continue;

      const KT_ElfW(Sym)* symtab = reinterpret_cast<KT_ElfW(Sym)*>(
          reinterpret_cast<char*>(mmap_info.data) + shdr[i].sh_offset);
      const size_t symCount = shdr[i].sh_size / shdr[i].sh_entsize;
      const KT_ElfW(Shdr)* strtabShdr = &shdr[shdr[i].sh_link];
      const char* strtab =
          reinterpret_cast<char*>(reinterpret_cast<char*>(mmap_info.data) + strtabShdr->sh_offset);
      ...
      for (size_t j = 0; j < symCount; ++j) {
        ...
        std::string sym_str =
            std::string(reinterpret_cast<const char*>(strtab + curr_sym->st_name));
        if (!sym_str.empty() && sym_str.data())
...
ElfScanner& ElfScannerMgr::getProgramElf() {
  ...
  if (!_programElf.isValid() || !_programElf.dynamic()) {
    ...
    // Make sure to always get native one
    {
      ...
      std::string binDir =
          KittyUtils::Path::fileDirectory(KittyUtils::Path::fileDirectory(exePath));
      exeMaps = KittyMemoryEx::getMaps(_pMem->processID(), KittyMemoryEx::EProcMapFilter::Equal,
                                       binDir + "/" + exeName, allMaps);
      if (exeMaps.empty()) {
        exeMaps = KittyMemoryEx::getMaps(_pMem->processID(), KittyMemoryEx::EProcMapFilter::Equal,
                                         exePath, allMaps);
      }
...
#endif
{
  ...
  static auto eMachineCheck = [](EScanElfType type, int a, int b) -> bool {
    return a == 0 || b == 0 || type == EScanElfType::Any ||
           (type == EScanElfType::Native && a == b) || (type == EScanElfType::Emulated && a != b);
  };
...
#ifdef __ANDROID__
std::vector<std::pair<uintptr_t, ElfScanner>> ElfScannerMgr::findSymbolAll(
    const std::string& symbolName, EScanElfType type, EScanElfFilter filter)
#else
std::vector<std::pair<uintptr_t, ElfScanner>> ElfScannerMgr::findSymbolAll(
...
uintptr_t ElfScannerMgr::findRemoteSymbol(const std::string& local_sym_name,
                                          uintptr_t local_sym_addr) {
...
#ifdef __ANDROID__
LinkerScannerMgr::LinkerScannerMgr(IKittyMemOp* pMem, uintptr_t linkerBase)
    : ElfScanner(pMem, linkerBase) {
...
LinkerScannerMgr::LinkerScannerMgr(IKittyMemOp* pMem, const ElfScanner& linkerElf)
    : ElfScanner(linkerElf) {
  memset(&_linker_syms, 0, sizeof(_linker_syms));
  memset(&_soinfo_offsets, 0, sizeof(_soinfo_offsets));
  _init = false;
...
bool LinkerScannerMgr::init() {
  ...
  KITTY_LOGD("soinfo_phdr(%zx, %zx) | soinfo_dyn(%zx)", _soinfo_offsets.phdr, _soinfo_offsets.phnum,
             _soinfo_offsets.dyn);
  KITTY_LOGD("soinfo_strtab(%zx, %zx) | soinfo_symtab(%zx)", _soinfo_offsets.strtab,
             _soinfo_offsets.strsz, _soinfo_offsets.symtab);

  if (!(_soinfo_offsets.size && _soinfo_offsets.bias && _soinfo_offsets.dyn &&
        _soinfo_offsets.symtab && _soinfo_offsets.strtab)) {
    return false;
  }
  ...
  for (size_t i = 0; i < solist_buf.size(); i += sizeof(uintptr_t)) {
    ...
    if (!KittyMemoryEx::getAddressMap(_pMem->processID(), possible_next + _soinfo_offsets.base,
                                      allMaps)
             .readable)
      continue;
    ...
    if (tmp_elf.isValid()) {
      if (!KittyMemoryEx::getAddressMap(_pMem->processID(), possible_next + _soinfo_offsets.size,
                                        allMaps)
               .readable)
        continue;
...
kitty_soinfo_t LinkerScannerMgr::infoFromSoInfo_(
    uintptr_t si, const std::vector<KittyMemoryEx::ProcMap>& maps) const {
  kitty_soinfo_t info{};
...
bool NativeBridgeScannerMgr::init() {
  ...
  _nbElf =
      _elfScanner->findElf("/libnativebridge.so", EScanElfType::Native, EScanElfFilter::System);
  if (!_nbElf.isValid()) {
  ...
  *(uintptr_t*)&fnNativeBridgeInitialized = _nbElf.findSymbol("NativeBridgeInitialized");
  if (fnNativeBridgeInitialized == nullptr)
    *(uintptr_t*)&fnNativeBridgeInitialized =
        _nbElf.findSymbol("_ZN7android23NativeBridgeInitializedEv");

  _nbImplElf = _elfScanner->findElf("/libhoudini.so", EScanElfType::Native, EScanElfFilter::System);
  if (_nbImplElf.isValid())
    _isHoudini = true;
  else
    _nbImplElf = _elfScanner->findElf("/libndk_translation.so", EScanElfType::Native,
  ...
  KITTY_LOGD("NativeBridgeScanner: Using nativebridge version (%d), data size (%p)",
             _nbItf_data.version, (void*)_nbItf_data_size);
...
#ifdef __LP64__
  LinkerScannerMgr emulinker = LinkerScannerMgr(
      _pMem, _elfScanner->findElf("/linker64", EScanElfType::Emulated, EScanElfFilter::System));
#else
  LinkerScannerMgr emulinker = LinkerScannerMgr(
      _pMem, _elfScanner->findElf("/linker", EScanElfType::Emulated, EScanElfFilter::System));
#endif
  ...
  } else  // Houdini
  {
    ...
    for (auto& sohead : soheads) {
      ...
      for (auto& it : _nbImplElf.segments()) {
        if (it.is_rw) {
          sohead.soinfo =
              _memScanner->findDataFirst(it.startAddress, it.endAddress, &data, sizeof(data));
          if (sohead.soinfo) {
            KITTY_LOGD("NativeBridgeScanner: Found sohead->phdr ref (%p) at %s",
                       (void*)sohead.soinfo, it.toString().c_str());
            break;
          }
        }
      }
      ...
      for (size_t i = 0; i < si_buf.size(); i += sizeof(uintptr_t)) {
        ...
        if (sohead.offsets.size && sohead.offsets.bias && sohead.offsets.dyn &&
            sohead.offsets.symtab && sohead.offsets.strtab) {
    ...
    for (auto& sohead : soheads) {
      if (sohead.soinfo_next_count > nmost) {
        ...
        KITTY_LOGD("NativeBridgeScanner: sohead (%d) %p -> %s", sohead.soinfo_next_count,
                   (void*)_sohead, _soheadElf.realPath().c_str());
      }
    }
  }
  ...
  KITTY_LOGD("nb_soinfo_base(%zx) | nb_soinfo_size(%zx) | nb_soinfo_bias(%zx)",
             _soinfo_offsets.base, _soinfo_offsets.size, _soinfo_offsets.bias);
  KITTY_LOGD("nb_soinfo_phdr(%zx, %zx) | nb_soinfo_dyn(%zx)", _soinfo_offsets.phdr,
             _soinfo_offsets.phnum, _soinfo_offsets.dyn);
  KITTY_LOGD("nb_soinfo_strtab(%zx, %zx) | nb_soinfo_symtab(%zx)", _soinfo_offsets.strtab,
             _soinfo_offsets.strsz, _soinfo_offsets.symtab);
...
kitty_soinfo_t NativeBridgeScannerMgr::infoFromSoInfo_(
    uintptr_t si, const std::vector<KittyMemoryEx::ProcMap>& maps) const {
  kitty_soinfo_t info{};
