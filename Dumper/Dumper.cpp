...
std::string Dumper::dumpClass(Il2Cpp::Class *klass)
{
    ...
    if (g().memory->isPtrValid(reinterpret_cast<uintptr_t>(this_type)))
    {
        ...
        result += g().il2cpp->getIsGeneric(klass) && g().memory->isPtrValid(reinterpret_cast<uintptr_t>(g().il2cpp->getThisType(klass))) ? g().il2cpp->getTypeName(g().il2cpp->getThisType(klass)) : g().il2cpp->getClassName(klass);
