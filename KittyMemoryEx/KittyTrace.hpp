 ...
 */
enum KT_HW_BP_TYPE { KT_HW_BP_EXECUTE = 0, KT_HW_BP_READ, KT_HW_BP_WRITE, KT_HW_BP_ACCESS };
...
#define KT_REGS_ALIGN_STACK(regs) \
  regs.KT_REG_SP = uintptr_t(intptr_t(intptr_t(regs.KT_REG_SP) & intptr_t(~0xF)))
...
/**
 * @brief A class for tracing and controlling the execution of processes in a
...
public:
  KittyTraceMgr()
      : _pid(0),
        ...
        _attached(false),
        _seized(false),
        _autoRestoreRegs(true),
        _remoteCallTimeout(0) {
  }
   ...
   * @param defaultCaller The default caller for remote function calls
   * (optional).
   * @param autoRestoreRegs Whether to automatically restore registers on remote
  ...
  KittyTraceMgr(pid_t pid, uintptr_t defaultCaller = 0, bool autoRestoreRegs = true,
      ...
      : _pid(pid),
        _defaultCaller(defaultCaller),
        _attached(isAttached()),
        _seized(false),
        _autoRestoreRegs(autoRestoreRegs),
        _remoteCallTimeout(remoteCallTimeout) {
  }
   ...
   * @return True if the signal information was retrieved successfully, false
   * otherwise.
   */
   ...
   * @param regs Pointer to the registers.
   * @param arg_num  The zero-indexed argument number (0 for 1st arg, 1 for 2nd,
   * etc.).
  ...
  /**
   * @brief Returns true if automatic registers restore for remote function
   * calls is enabled, false otherwise.
   */
   ...
   */
  template <class... Args>
  kitty_rp_call_t callFunctionFrom(uintptr_t callerAddress, uintptr_t functionAddress,
                                   Args&&... a) {
    return _callFunctionFrom(callerAddress, functionAddress, sizeof...(a),
                             std::forward<Args>(a)...);
  }
  ...
  kitty_rp_call_t callFunction(uintptr_t functionAddress, Args&&... a) {
    return _callFunctionFrom(_defaultCaller, functionAddress, sizeof...(a),
                             std::forward<Args>(a)...);
  }

  /**
   * @brief Call a syscall in the remote process.
   * @note This function writes syscall + brkp instructions at PC/IP so PC/IP
   * must be at valid executable address.
   ...
   * @param timeout_ms timeout in milliseconds (0 or negative value, will
   * disable timeout)
   ...
   */
  KT_BP_RESULT
  setSoftBreakpointAndWait(uintptr_t address, const std::function<bool(user_regs_struct regs)>& cb,
                           int timeout_ms);

  /**
   * @brief Sets and wait for hardware breakpoint at a given address.
   * @param address The address to set the breakpoint at.
   * @param type The type of hardware breakpoint (e.g., execute, read, write,
   * access).
   * @param size The size of the data to watch (ignored if bp type is
   * KT_HW_BP_EXECUTE).
   ...
   */
  KT_BP_RESULT setHardBreakpointAndWait(uintptr_t address, KT_HW_BP_TYPE type, KT_HW_BP_SIZE size,
                                        int slot,
  ...
  /**
   * @brief Clears a hardware breakpoint on a specified address for traced
   * thread.
   *
   * @param type The type of hardware breakpoint (e.g., execute, read, write,
   * access).
