  ...
  if (initMemPatch) {
    if (eMemOp == EK_MEM_OP_IO) {
      ...
      if (_pMemOpPatch->init(pid)) {
        ...
        KITTY_LOGW(
            "KittyMemoryMgr: Couldn't initialize IO memory operation "
            "for memory patching.");
      }
    }
  }
...
#ifdef __LP64__
  linkerScanner = LinkerScannerMgr(
      _pMemOp.get(), elfScanner.findElf("/linker64", EScanElfType::Native, EScanElfFilter::System));
#else
  linkerScanner = LinkerScannerMgr(
      _pMemOp.get(), elfScanner.findElf("/linker", EScanElfType::Native, EScanElfFilter::System));
#endif
...
bool KittyMemoryMgr::dumpMemRange(uintptr_t start, uintptr_t end,
                                  const std::string& destination) const {
  if (!isMemValid())
    return false;

  if (start >= end) {
    KITTY_LOGE("dumpMemRange: start(%p) is equal or greater than end(%p).", (void*)start,
               (void*)end);
    return false;
  }
